...
func (sq *ServerQuery) QueryConfiguration() *ServerConfigQuery {
...
// QueryWordBlacklist chains the current query on the "word_blacklist" edge.
func (sq *ServerQuery) QueryWordBlacklist() *WordBlacklistQuery {
	query := (&WordBlacklistClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *ServerQuery) WithConfiguration(opts ...func(*ServerConfigQuery)) *ServerQuery {
	query := (&ServerConfigClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withConfiguration = query
	return sq
}
...
func (sq *ServerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Server, error) {
	...
	if query := sq.withConfiguration; query != nil {
		if err := sq.loadConfiguration(ctx, query, nodes, nil,
			func(n *Server, e *ServerConfig) { n.Edges.Configuration = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := sq.withWordBlacklist; query != nil {
		if err := sq.loadWordBlacklist(ctx, query, nodes,
			...
			func(n *Server, e *WordBlacklist) { n.Edges.WordBlacklist = append(n.Edges.WordBlacklist, e) }); err != nil {
			return nil, err
		}
	}
...
func (sq *ServerQuery) loadWordBlacklist(ctx context.Context, query *WordBlacklistQuery, nodes []*Server, init func(*Server), assign func(*Server, *WordBlacklist)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(server.WordBlacklistTable)
		s.Join(joinT).On(s.C(wordblacklist.FieldID), joinT.C(server.WordBlacklistPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(server.WordBlacklistPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(server.WordBlacklistPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*WordBlacklist](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "word_blacklist" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
