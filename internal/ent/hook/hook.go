...
// function as WordBlacklist mutator.
type WordBlacklistFunc func(context.Context, *ent.WordBlacklistMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f WordBlacklistFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.WordBlacklistMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.WordBlacklistMutation", m)
}
